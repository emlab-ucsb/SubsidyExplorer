...
eez_fao_left <- (st_geometry(eez_fao_simple) - c(360, 0)) %>%
  st_set_crs(4326)

# Make right duplicate (180, 540)
eez_fao_right <- (st_geometry(eez_fao_simple) + c(360, 0)) %>%
  st_set_crs(4326)

# Assign geometry
eez_fao_left <- st_set_geometry(eez_fao_simple, eez_fao_left)
eez_fao_right <- st_set_geometry(eez_fao_simple, eez_fao_right)

# EEZ fao all <
eez_fao_all <- eez_fao_simple %>%
  rbind(eez_fao_left) %>% 
  rbind(eez_fao_right) %>%
  st_make_valid() %>%
