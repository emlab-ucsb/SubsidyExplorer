...
while (t < stop_time) {
  ...
  if(length(managed_fleets) > 0){
    
    all_man <- c(aff_man, un_man)
    
    effort_fleet_bau[t+1,all_man] <- effort_fleet_bau[t,all_man]
    # effort_fleet_bau[t+1,all_man] <- pmax(0, pmin(effort_fleet_bau[1,all_man], (eta*profits_fleet_bau[t,all_man] + effort_fleet_bau[t,all_man])))
    
  }
  # Fleets with unlimited effort (open effort)
  if(length(open_access_fleets) > 0){
  ...
  #   
  #   # Fleet 1 is managed
  #   effort_fleet_bau[t+1,1] <- max(0, min(effort_fleet_bau[1,1], (eta*profits_fleet_bau[t,1] + effort_fleet_bau[t,1])))
  #   # Fleet 2 is open access
  #   effort_fleet_bau[t+1,2] <- max(0, (eta*profits_fleet_bau[t,2] + effort_fleet_bau[t,2]))
  #   
  # }
  # 
  ...
  # if(catches_fleet_bau[t+1,] == 0){
  #   browser()
  # }

  # catches_fleet_bau[t+1,nf] <- max(0, (q2*biomass_bau[t,2]*effort_fleet_bau[t+1,nf]))
  ...
  #revenue_fleet_bau[t+1,nf] <- price_bau[t+1,2]*catches_fleet_bau[t+1,nf]
...
bau_out <- list(biomass = biomass_bau,
                #biomass_total = biomass_total_bau,
                price = price_bau,
...
# biomass_total <- matrix(0, nrow = stop_time, ncol = 1)
# rownames(biomass_total) <- year_range
...
## Solve for cost coefficients - same used throughout
# cost_coeff <- (revenue_fleet[1,]+(subsidies_fleet[1,]*effort_fleet[1,]))/(effort_fleet[1,]^beta)
# cost_coeff[is.nan(cost_coeff)] <- 0
# done above
...
## Solve for catchability
# q <- catches_fleet[1,1:nf-1]/(biomass[1,1]*effort_fleet[1,1:nf-1])
# q[is.nan(q)] <- 0
# q2 <- catches_fleet[1,nf]/(biomass[1,2]*effort_fleet[1,nf])
# q2[is.nan(q2)] <- 0
# done above
...
while (t < stop_time) {
  ...
  # biomass[t+1,2] <- max(0, (biomass[t,2] + ((phi + 1)/phi)*r*biomass[t,2]*(1-((biomass[t,2]/K_remain))^phi) - sum(catches_fleet[t,nf])))
  # 
  # biomass[t+1,][is.nan(biomass[t+1,])] <- 0
  
  #biomass_total[t+1] <- sum(biomass[t+1,])
  ...
  if(length(managed_fleets) > 0){
    ...
    effort_fleet[t+1,all_man] <- effort_fleet[t,all_man]
    # effort_fleet[t+1,all_man] <- pmax(0, pmin(effort_fleet[1,all_man], (eta*profits_fleet[t,all_man] + effort_fleet[t,all_man])))
  ...
  # if(nf==1){
  #   
  #   # Everyone is open access
  #   effort_fleet[t+1,] <- max(0, (eta*profits_fleet[t,] + effort_fleet[t,]))
  #   
  # }else if(nf == 2){
  #   
  #   # Fleet 1 is managed
  #   effort_fleet[t+1,1] <- max(0, min(effort_fleet[1,1], (eta*profits_fleet[t,1] + effort_fleet[t,1])))
  #   # Fleet 2 is open access
  #   effort_fleet[t+1,2] <- max(0, (eta*profits_fleet[t,2] + effort_fleet[t,2]))
  #   
  # }
  ...
  #catches_fleet[t+1,nf] <- max(0, (q2*biomass[t,2]*effort_fleet[t+1,nf]))
  
  catches_total[t+1] <- sum(catches_fleet[t+1,])
  
  # Price
  price[t+1] = max(0, ((1/delta)^(1/epsilon))*(sum(catches_fleet[t+1,])^(1/epsilon)))
  # price[t+1,2] = max(0, ((1/delta)^(1/epsilon))*(sum(catches_fleet[t+1,nf])^(1/epsilon)))
  # price[t+1,][is.infinite(price[t+1,])] <- 0
  
  ### Revenue 
  revenue_fleet[t+1,] <- price[t+1]*catches_fleet[t+1,]
  # revenue_fleet[t+1,nf] <- price[t+1,2]*catches_fleet[t+1,nf]
