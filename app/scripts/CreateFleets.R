...
CreateFleets <- function(vessel_list,
  ...
  vessel_list <- vessel_list %>%
    rename(fishing_h = fishing_hours_eez_fao,
           fishing_KWh = fishing_KWh_eez_fao)
  
  ### Summary statistics by flag ---
  flag_summary <- bind_cols(
    ...
    (vessel_list %>%
       group_by(flag) %>%
       summarize_at(c("fishing_h", "fishing_KWh", "catch", "revenue", "good_subs", "bad_subs", "ugly_subs", paste0(subsidy_types_all, "_subs")), sum, na.rm = T))
  ) %>%
    ungroup() %>%
    select(-flag1) %>%
  ...
  eu_summary <- bind_cols(
    ...
    # Summarize general variables
    (vessel_list %>%
       dplyr::filter(flag %in% eu_countries) %>%
       summarize(n_vessels = n_distinct(ssvid),
                 n_vessel_class = n_distinct(vessel_class),
                 avg_length_m = mean(length_m, na.rm = T),
                 avg_tonnage_gt = mean(tonnage_gt, na.rm = T),
  ...
  if("iuu2" %in% iuu$definitions | "iuu3" %in% iuu$definitions | "iuu4" %in% iuu$definitions){
  ...
  ### Developing countries and LDCs    
  }else if(iuu$sdt_who == "developing"){
    ...
    if("all" %in% iuu$sdt_what){
      ...
      # If all Member-flagged vessels are exempt... This trumps all other options
      iuu_vessels_sd <- iuu_vessels_sdt
  ...
  ### States with small and vulnerable economies only  
    ...
    if("all" %in% iuu$sdt_what){
      ...
      # Exempt domestic fishing vessels (less than 1% of effort)
      iuu_vessels_sdt <- iuu_vessels_sdt %>%
        dplyr::filter(prop_fishing_KWh_HS < 0.01)
  ...
  if("OA2" %in% oa$definitions){
    ...
    #Is considered to be overfished (B/Bmsy < 0.8 or similar metric) as determined by the most recent data-limited stock assessment" = "OA2"
  ...
  if(nrow(oa_vessels) >= 1 & oa$scope != "all"){
    ...
    if(oa$scope == "HS"){
      ...
      oa_vessels_scope <- oa_vessels_scope %>%
        dplyr::filter(prop_fishing_KWh_HS >= oa_hs_cutoff)
    ...
    }else if(oa$scope == "OUT"){
      
      oa_hs_cutoff <- oa$hs_cutoff/100
      
      # High seas or distant water fishing only
      oa_vessels_scope <- oa_vessels_scope %>%
        dplyr::filter(distant_water | prop_fishing_KWh_HS >= oa_hs_cutoff)
  ...
  if(nrow(oa_vessels_scope) >= 1 & oa$allow_sdt == "Yes"){
    ...
    if(oa$sdt_who == "ldc"){
      ...
      oa_vessels_sdt <- oa_vessels_sdt %>%
        dplyr::filter(development_status == "LDC")
      
      if("all" %in% oa$sdt_what){
        
        # If all Member-flagged vessels are exempt... This trumps all other options
        oa_vessels_sdt <- oa_vessels_sdt
        
      }else if("domestic" %in% oa$sdt_what){
        
        # Exempt domestic fishing vessels (less than 1% of fishing effort)
        oa_vessels_sdt <- oa_vessels_sdt %>%
          dplyr::filter(prop_fishing_KWh_HS < 0.01) 
        
      }else if("HS" %in% oa$sdt_what){
        
        # Exempt vessels fishing on the high seas
        oa_hs_cutoff <- oa$sdt_hs_cutoff/100
