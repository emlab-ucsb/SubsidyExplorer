...
BioEconomicModelPlots <- function(dat,
  ...
  if(temporal_res == "all"){
    ...
    plot <- plot_dat %>%
      ...
      ggplot()+
      aes(x = Year, y = Diff_tot*100, color = Variable)+
      geom_line(size = 1)+
    ...
    plot_dat <- plot_dat %>%
      ...
      group_by(Year, Variable, Fleet, Region) %>%
    ...
    plot <- plot_dat %>%
      ggplot()+
      aes(x = Biomass, y = Catches, size = Revenue, color = Revenue)+
      geom_point()+
      ...
      scale_size(breaks = seq(rev_limits[1], rev_limits[2], length.out = 5),
                 labels = percent(seq(rev_limits[1], rev_limits[2], length.out = 5)),
                 limits = rev_limits)+
      guides(size = guide_legend(reverse = T, title = NULL))+
